...
const int MPPT_PWM_CHANNEL = 0; // Kanal 0 (Achtung: Prüfen ob LEDC woanders Kanal 0 nutzt, sonst 1 nehmen)
...
void Task_MPPT(void* pvParameters) {
    ...
    // Startzustand: Wir fangen "sanft" an (Hohe Injektion = Wenig Last für das Panel)
    ...
    while (1) {
        ...
        if (dataValid) {
            // --- SICHERHEITS-CHECK ---
            // Wenn Spannung zu tief einbricht (Panel kollabiert), sofort Last wegnehmen!
            if (currentSolarVoltage < 9.0) { 
                ...
                if (abs(powerDiff) > 10.0) { // > 10mW Änderung
